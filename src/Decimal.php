...
use Sibosend\BigNumbers\Errors\InfiniteInputError;
use Sibosend\BigNumbers\Errors\NaNInputError;
...
//            return self::fromFloat($value, $scale);
            throw new InvalidArgumentTypeError(['integer', 'string', 'Decimal'], $value);
     ...
     * @return Decimal
     */
    private static function fromInteger($intValue)
    {
        self::paramsValidation($intValue, null);

        if (!is_int($intValue)) {
            throw new InvalidArgumentTypeError(['integer'], $intValue, '$intValue must be of type int');
        }
     ...
     * @return Decimal
     */
    private static function fromFloat($fltValue, $scale = null)
    {
        self::paramsValidation($fltValue, $scale);

        if (!is_float($fltValue)) {
            throw new InvalidArgumentTypeError(['double'], $fltValue, '$fltValue must be of type float');
        }
     ...
     * @return Decimal
     */
    private static function fromString($strValue, $scale = null)
    {
        self::paramsValidation($strValue, $scale);

        if (!is_string($strValue)) {
            throw new InvalidArgumentTypeError(['string'], $strValue, '$strValue must be of type string.');
        }
...
//        }
        ...
        $scale = isset($scale) ? $scale : $min_scale;
     ...
     *
     * @param  mixed|Decimal $b exponent
     * @param  integer $scale
     * @return Decimal
     */
    public function pow($b, int $scale = null)
    {
        ...
        if ($this->isZero()) {
            ...
            $pow_scale = max($this->scale, $b->scale, isset($scale) ? $scale : 0);
    ...
    )
    {
        ...
        $value = (
            ...
            bcmul(
                $mantissa,
                $tmp_multiplier,
                max($minScale, isset($scale) ? $scale : 0)
            )
        );
     ...
     * @param  null|int $scale
     */
    protected static function paramsValidation($value, $scale = null)
    {
        if (null === $value) {
            throw new InvalidArgumentTypeError(null, $value, '$value must be a non null number');
        }

        if (null !== $scale&& (!is_int($scale) || $scale < 0)) {
            throw new InvalidArgumentTypeError(null, $scale, '$scale must be a positive integer');
        }
    }
